...
public record River(
    ...
    public static final Codec<River> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Codec.STRING.optionalFieldOf("name", "").forGetter(River::name),
    ...
    /**
     * Distance from a block position to the nearest point on this river's (meandered) spline.
     * Backed by {@link RiverGenerator}'s per-river spatial index, which is computed once per
     * river and reused — calling this repeatedly (e.g. per-block) does not re-walk the spline.
     */
    public double distanceTo(double blockX, double blockZ) {
        return RiverGenerator.distanceToRiver(this, blockX, blockZ);
    }
            ...
            for (var road : network.roads()) {
                ...
                for (int i = 0; i < wps.size() - 1; i++) {
                    ...
                    for (int s = 0; s <= steps; s++) {
                        double t2 = (double) s / steps;
                        double rx = from.x() + (to.x() - from.x()) * t2;
                        double rz = from.z() + (to.z() - from.z()) * t2;
                        double dx = blockX - rx;
                        double dz = blockZ - rz;
